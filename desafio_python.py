...
def carrega_dados():
    ...
    dados_csv.rename(columns= {'AvgTemperature': 'Temperatura Média'} ,inplace=True)
    dados_csv.rename(columns= {'Region': 'Continente'} ,inplace=True)
...
with st.container(): #Trtamento dos dados para os gráficos
    dados = carrega_dados()
    dados1 = carrega_dados()[["Country", "City","Day", "Temperatura Média"]]
    clima2 = carrega_dados()[["Country", "City","Year","Month", "Day", "Temperatura Média"]]
    clima3 = carrega_dados()
    with st.sidebar: #Criação da Sidebar
        st.title("Temperatura Mundial")
        with st.container(): #Region
            region = st.selectbox("Selecione o Continente", ["Africa", "Asia", "Australia/South Pacific", "Europe", "Middle East", "North America", "South/Central America"])
            clima = carrega_dados().loc[carrega_dados()['Continente'] == region]
        with st.container(): #Country
            dCity = clima[['Continente', 'Country']]
            ta1 = dCity.loc[dCity["Continente"] == region]
            df = pd.DataFrame(ta1)
            df = df.drop_duplicates()
            df = df.sort_values('Country', ascending=True)
            lista = df['Country'].tolist()
            country = st.selectbox("Selecione o País",lista )
    ...
    clima = clima [["Country", "City","Year","Month", "Day", "Temperatura Média"]]
    clima2 = clima2 [["Country", "City","Year","Month", "Day", "Temperatura Média"]]
    clima4 = clima4 [["Country", "City","Year","Month", "Day", "Temperatura Média"]]
    clima5 = clima5 [["Country", "City","Year","Month", "Day", "Temperatura Média"]]
with st.container(): #Gráfico de barras
        grafico1= px.bar(clima, x = "Day", y = "Temperatura Média", title=f'Temperatura no mes de {month} do ano de {year}')
        grafico1.update_layout(width=400, height=300)  
with st.container(): #Gráfico de dispersão
        grafico2 = px.scatter(clima5, x="Temperatura Média", y="Day", color="Year", hover_name="Country", log_x=True, size_max=60, title='Evolução da Temperatura ao longo dos anos')
...
with st.container(): #grafico de pizza
    ...
    df_pie = df.groupby('Continente')['City'].nunique().reset_index()
    grafico3 = px.pie(df_pie, names='Continente', values='City', title=f'Percentual de dados coletados por Região em {year}')
...
with st.container(): #Map de Temperatura Média por País
    ...
    df_country = clima2.groupby('Country')['Temperatura Média'].mean().reset_index()
    # Criar o Choropleth Map
    grafico4 = px.choropleth(df_country, locations="Country", locationmode="country names",color="Temperatura Média", hover_name="Country", color_continuous_scale=px.colors.sequential.Plasma,title= f"Temperatura Média por País em {year}",template= "gridon")
    grafico4.update_layout(width=500, height=500)
with st.container():
    df = carrega_dados()
    df = df[df['Year'] == year]  # Filtrar por ano para reduzir o tamanho do dataframe
    #Agrupar os dados
    df = df.groupby(['Continente', 'Year','Month', 'Day'])['Temperatura Média'].mean().reset_index()
    df['Date'] = pd.to_datetime(df[['Year', 'Month', 'Day']])
    grafico5 = px.line(df, x='Date', y='Temperatura Média', color='Continente', title=f'Evolução da Temperatura Média ao longo dos messes do ano de {year}')
